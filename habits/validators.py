    ...
    def __call__(self, value, *args, **kwargs):
        if value.get('is_pleasant'):
            if value.get('prize') or value.get('related_habit'):
                raise ValidationError('Для приятной привычки '
            ...
            if value.get('prize') and value.get('related_habit'):
                raise ValidationError('Нужно выбрать или '
                                      'вознаграждение или приятную привычку'
                                      )
            if (value.get('related_habit') is None
                    and value.get('prize') is None):
                raise ValidationError('Нужно выбрать или '
                                      'вознаграждение или приятную привычку'
                                      )
        ...
        if value.get('is_pleasant'):
            ...
            if value.get('execution_time'):
                ...
                if value.get('execution_time') < 1:
                    raise ValidationError('Время выполнения привычки '
                                          'должно быть больше 1 секунды'
                                          )
            else:
                raise ValidationError('Вы не заполнили поле с временем '
                                      'выполнения привычки (execution_time)'
                                      )


class IsRelatedHabitPleasant:
    """Проверка на корректность привязки к
    полезным привычкам только приятных"""
    def __init__(self):
        pass
    ...
    def __call__(self, value, *args, **kwargs):
        if value.get('related_habit') and not value.get('is_pleasant'):
            if not value.get('related_habit').is_pleasant:
                raise ValidationError('Полезные привычки можно '
                                      'связывать только с приятными'
                                      )
...
class HabitPeriodValidator:
    ...
    def __call__(self, value, *args, **kwargs):
        if value.get('is_pleasant'):
            if value.get('period'):
                raise ValidationError('Для приятных привычек '
                                      'не нужна периодичность'
                                      )
        else:
            if value.get('period'):
                if value.get('period') > 7:
                    raise ValidationError('Привычку нужно повторять '
                                          'не реже раза в неделю'
                                          )
                if value.get('period') < 1:
                    raise ValidationError('Периодичность указана некорректно, '
                                          'она должна быть от одного до 7 дней'
                                          )
            else:
    ...
    def __call__(self, value, *args, **kwargs):
        if value.get('is_pleasant'):
            if value.get('habit_time'):
                raise ValidationError("Приятной привычке не "
                                      "требуется время (habit_time)"
                                      )
        else:
            if not value.get('habit_time'):
                raise ValidationError("Вы не заполнили "
                                      "время привычки (habit_time)"
                                      )
